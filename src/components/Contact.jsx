    ...
    <section id="contact" className="py-12 fade-in">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transform transition duration-300 hover:scale-105 hover:shadow-xl">
        <p className="text-gray-600 dark:text-gray-400 flex flex-col md:flex-row items-center md:items-start text-center md:text-left break-words">
          ...
          <a href={`mailto:${data.contact.email}`} className="text-blue-500 hover:underline ml-1 break-all">
            {data.contact.email}
          </a>
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 animate-slide-up">
          <div>
            <label htmlFor="name" className="block text-gray-700 dark:text-gray-300">Your Good Name Please</label>
            <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required className="w-full p-2 mt-1 border rounded dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200" placeholder="Your Name" />
          </div>
          <div>
          ...
          <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition-colors duration-200 transform hover:scale-105">Send Message</button>
          {status && <p className={`mt-2 text-sm ${status.includes("successfully") ? "text-green-500" : "text-red-500"}`}>{status}</p>}
        </form>
      </div>
    </section>
  );
